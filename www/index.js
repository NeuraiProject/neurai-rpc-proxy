...
// DePIN: one request is enough to show the shape — the same POST as any other
// method. Section names are public chain data, so this call needs no proof.
function depinExample(endpoint) {
  return `const response = await fetch("${endpoint}", {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ method: "depinlistsections", params: [] })
});

// result: { body: "<hex JSON>", poolsig: "<base64>" }
// poolsig is the node's pool-key signature; body decodes to
// { sections: [{ name: "&NEWS/GENERAL", label: "GENERAL", depth: 1 }, …] }
const { result } = await response.json();`;
}
